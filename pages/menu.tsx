    ...
    <div className="flex flex-col gap-12 pt-8 pb-16">
      ...
      <section className="w-full relative px-4 md:px-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto">
          {combos.map((combo) => (
            <Link key={combo.id} href={`/menu/${combo.id}`} className="group block focus:outline-none">
              ...
              <div className="w-full aspect-[4/3] bg-black/5 relative overflow-hidden flex-shrink-0">
                {/* Fallback texture if image is missing */}
                <div className="absolute inset-0 bg-[#c5b092] flex items-center justify-center">
                   <span className="text-[#5E3B22] opacity-30 font-bold uppercase tracking-widest text-xl">Hình Ảnh</span>
                </div>
                <img src={combo.image} alt={combo.title} className="absolute inset-0 w-full h-full object-cover z-0" />
                ...
                <div className="absolute -bottom-[2px] left-0 right-0 w-full overflow-hidden leading-none z-10 block">
                  <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="w-full h-5 md:h-7 text-[#e4ccaa]" fill="currentColor">
              ...
              {/* Price Circle Badge */}
              <div className="absolute right-[2%] top-[calc(55%_-_10px)] md:top-[calc(60%_-_32px)] -translate-y-1/2 w-20 h-20 md:w-24 md:h-24 bg-[#A9442A] rounded-full flex flex-col items-center justify-center z-20 shadow-[0_4px_10px_rgba(0,0,0,0.4)]" style={{ border: '3px solid #DFB58C' }}>
                <span className="text-[#FFEBB5] font-bold text-[9px] md:text-[10px] tracking-widest uppercase mb-0.5" style={{ fontFamily: 'var(--font-sans)' }}>Giá</span>
                <span className="text-[#FFEBB5] font-bold text-sm md:text-base leading-tight" style={{ fontFamily: 'var(--font-heading)' }}>{combo.price}</span>
                <span className="text-[#FFEBB5] font-bold text-[9px] md:text-[10px] leading-none mt-0.5" style={{ fontFamily: 'var(--font-heading)' }}>VNĐ</span>
              </div>
